...
module.exports = class {
    ...
    async run (message) {
        ...
        const guildConf = this.client.settings.get(message.guild.id,"language")
        data.guildConf = guildConf;
        data.config = this.client.config;

        // Gets language
        let Language = require(`../languages/${guildConf}.js`);
        message.language = new Language();

        // Check if the bot was mentionned
        const prefixMention = new RegExp(`^<@!?${this.client.user.id}>( |)$`);
        if(message.content.match(prefixMention)) return message.reply(message.language.get("PREFIX_INFO", "log!"));

        // Gets the prefix
        let prefixes = [ "log!", this.client.user.username, "<@"+this.client.user.id+">" ];
        ...
        this.client.infos = this.client.stats.clientInfos(this.client,message)
        if(message.guild){
            ...
            if(!message.channel.nsfw && cmd.conf.nsfw){
                return message.channel.send(message.language.get("ERR_CMD_NSFW"));
            }
        }

        if(!cmd.conf.enabled){
            return message.channel.send(message.language.get("ERR_CMD_DISABLED"));
        }
         /* PREMIUM */
        if(cmd.conf.premiumOnly && data.config.guildPrem.indexOf(message.guild.id) === -1){
