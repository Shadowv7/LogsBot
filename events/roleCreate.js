...
module.exports = class {
  ...
  async run(role) {
    ...
    if (this.client.logs.get(role.guild.id, `${events}.channel_id`)) {
     ...
     let emit_length = this.client.logs.get(role.guild.id,`${events}.emit_length`)
    emit_length+=1
    this.client.logs.set(role.guild.id,emit_length,`${events}.emit_length`)
        const userTag = await role.guild.fetchAuditLogs({type: 'ROLE_CREATE'}).then(audit => audit.entries.first().executor.tag)
