...
module.exports = class {
  ...
  async run(guild, user) {
    ...
    this.client.logs.set(guild.id,emit_length,`${events}.emit_length`)
       const userTag = await guild
        .fetchAuditLogs({ type: "MEMBER_BAN_DELETE" })
        .then(audit => audit.entries.first().executor.tag);
      
      const language = new(require(`../languages/${this.client.settings.get(guild.id,"language")}.js`))
      return channel.send(language.get(events, guild, user,userTag));
    }
  }
};
